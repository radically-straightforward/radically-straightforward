    ...
    const caddyChildProcess = childProcess.spawn(
      path.join(
        path
          ...
          .split(`${path.sep}node_modules${path.sep}`)[0],
            ...
            (untrustedStaticFilesRoot) => caddyfile`
              route {
                ...
                route @file_exists {
                  ...
                  @allow_cross_origin path *.webp *.webm *.png *.jpg *.jpeg *.gif *.mp3 *.wav *.mp4 *.m4v *.ogg *.mov *.mpeg *.avi
                  header @allow_cross_origin Cross-Origin-Resource-Policy cross-origin
                  file_server
                }
              }
            `,
