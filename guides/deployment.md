...
- A server. If you don’t have one, we recommend renting one from [Hetzner](https://www.hetzner.com/), which we use for our applications.
- A domain. If you don’t have one, we recommend registering one from [Namecheap](https://www.namecheap.com/), which we use for our applications.
...
2. On your server, download the application, configure it, and start it:
   ...
   # rm EXAMPLE-APPLICATION--ubuntu--v2.0.1.tar.gz
   ...
   # nano /etc/systemd/system/EXAMPLE-APPLICATION.service
...
```console
$ rsync -az --delete --progress root@EXAMPLE-APPLICATION.com:EXAMPLE-APPLICATION/data/ /path/to/backup/
```
